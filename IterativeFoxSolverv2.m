        ...
        if x >= min(X) + gridsize_x * (i-1) & x < min(X) + gridsize_x * i
            for j = 1:resy
                if y >= min(Y) + gridsize_y * (j-1) & y < min(Y) + gridsize_y * j
                    for k = 1:resz
                        if z >= min(Z) + gridsize_z * (k-1) & z < min(Z) + gridsize_z * k
                            i
                            j
                            k
...
% for i = 1:100
%     u = jacobi3Dv2(u, b, gridsize_x, gridsize_y, gridsize_z);
% %     u = gauss3Dv2(u, b, gridsize_x, gridsize_y, gridsize_z);
% end

%% step3.5 another option: use Multigrid Solver method to solve Poisson equation
for i = 1:50
    u = multigrid3Dv2(u, b, gridsize_x, gridsize_y, gridsize_z);
end
