...
gt_grid1 = zeros(n_basis, n_basis, n_basis);
gt_grid2 = zeros(n_basis, n_basis, n_basis);
gt_grid3 = zeros(n_basis, n_basis, n_basis);
gt_grid4 = zeros(n_basis, n_basis, n_basis);
gt_grid5 = zeros(n_basis, n_basis, n_basis);
gt_grid6 = zeros(n_basis, n_basis, n_basis);
                ...
                if y >= min(Y) + gridsize * (j-1) & y < min(Y) + gridsize * j
                    for k = 1:resz
                        if z >= min(Z) + gridsize * (k-1) & z < min(Z) + gridsize * k
                            ...
                            j
                            k 
                            gt_grid1(i,j,k) = gt_grid1(i,j,k) + 1;
                            gt_grid1(i,j,k+1) = gt_grid1(i,j,k+1) + 1;
                            gt_grid1(i,j+1,k) = gt_grid1(i,j+1,k) + 1;
                            gt_grid1(i+1,j,k) = gt_grid1(i+1,j,k) + 1;
                            gt_grid1(i,j+1,k+1) = gt_grid1(i,j+1,k+1) + 1;
                            gt_grid1(i+1,j+1,k) = gt_grid1(i+1,j+1,k) + 1;
                            gt_grid1(i+1,j,k+1) = gt_grid1(i+1,j,k+1) + 1;
                            gt_grid1(i+1,j+1,k+1) = gt_grid1(i+1,j+1,k+1) + 1;
...
%% step5 numerical error measurement, and finding isovalue
gt_grid = gt_grid1;
slice1_gt = squeeze(gt_grid(n_intervals/2,:,:));
slice2_gt = squeeze(gt_grid(:,n_intervals/2,:));
slice3_gt = squeeze(gt_grid(:,:,n_intervals/2));
figure(4)
heatmap(slice1_gt);
figure(5)
heatmap(slice2_gt);
figure(6)
heatmap(slice3_gt);
save('gt_grid.mat', 'gt_grid');

figure(7)
error = slice2_gt - slice2 / mean(slice2, 'all') * mean(slice2_gt, 'all');
heatmap(error);
